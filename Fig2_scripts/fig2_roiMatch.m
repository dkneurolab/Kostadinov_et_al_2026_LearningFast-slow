...
%% Load datasets
if ~exist(fullfile(figFold,'matchDataBehav.mat'),'file')
    ...
    if savebool
        save(fullfile(figFold,'matchDataBehav.mat'),'matchData','-v7.3');        
        save(fullfile(figFold,'matchDataFOVs.mat'),'matchFOVs');
    end
    
else
    matchData = load(fullfile(figFold,'matchDataBehav.mat')); matchData = matchData.matchData;
    matchFOVs = load(fullfile(figFold,'matchDataFOVs.mat')); matchFOVs = matchFOVs.matchFOVs;
end
...
for i = 1:numel(matchData)
    if ~isempty(matchData(i).name)
        ...
        if savebool
            ...
            print(figs(5),fullfile(figFoldfov1,[figname5,'.eps']), '-depsc', '-vector');
            ...
            print(figs(6),fullfile(figFoldfov1,[figname6,'.eps']), '-depsc', '-vector');
            
            figname7 = '07_exTraces';
            savefig(figs(7), fullfile(figFoldfov1,[figname7,'.fig']));
            saveas(figs(7),fullfile(figFoldfov1,[figname7,'.png']));
            print(figs(7),fullfile(figFoldfov1,[figname7,'.eps']), '-depsc', '-vector');            
        end
    end
end
