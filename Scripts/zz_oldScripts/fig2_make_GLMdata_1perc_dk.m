function [GLMdata, GLMholdout,DMboxPCA,trialbasis] = fig2_make_GLMdata_1perc_dk(DMboxdata, trialbasis, mzones, xvalfold,trialout)
...
[DMboxC, ~] = makeDesignMatrix_dk(trialbasis, (1:numel(trialbasis.expt))', 'bsbox');
[DMboxPCA.coeff,DMboxPCA.score,DMboxPCA.latent,DMboxPCA.tsquared,DMboxPCA.explained,DMboxPCA.mu] = pca(DMboxC);
figure; plot(DMboxPCA.explained)
% nPCs = sum(DMboxPCA.explained > 1);
nPCs = find(cumsum(DMboxPCA.explained) > 80,1);
DMboxPCA.nPCs = nPCs;

figure; 
subplot(2,1,1);
imagesc(DMboxPCA.score(1:1000,1:nPCs)'); colormap redblue
...
%     trainRew = iRews(~ismember(iRews,testRew));
    ...
    DMtestbox0 = DMtestbox0trial*DMboxPCA.coeff;
    DMtrainbox0 = DMtrainbox0trial*DMboxPCA.coeff;
    
    % Now only take nPCs > 1% EV
    DMtestbox = DMtestbox0(:,1:nPCs);
    DMtrainbox = DMtrainbox0(:,1:nPCs);
...
[DMholdBoxTrial0, DMholdBoxData] = makeDesignMatrix_dk(trialbasis, iHoldTrials, 'bsbox');
% [DMholdRew] = makeDesignMatrix_dk(rewbasis, iHoldRews, 'bs');
% [DMholdBoxRew] = makeDesignMatrix_dk(rewbasis, iHoldRews, 'bsbox');
DMholdBoxTrial = DMholdBoxTrial0*DMboxPCA.coeff;
DMholdBoxTrial = DMholdBoxTrial(:,1:nPCs);
...
GLMholdout.DMtrialBox = DMholdBoxTrial0(:,iBoxSmall);
% GLMholdout.DMrewBox = GLMholdout.DMrewBox_full(:,iBoxSmall);

for i = 1:xvalfold
    GLMdata(i).DMtestbox = GLMdata(i).DMtestbox;
%     GLMdata(i).DMtestbox = GLMdata(i).DMtestbox(:,iBoxSmall);    
    GLMdata(i).DMtrainbox = GLMdata(i).DMtrainbox;
