function fig3_matchFOVs_v44(sessionsv1,sessionsv4_n,sortFOV,dataFold,paperFold,savebool)
...
figV_OnMovRew = fig3_matchFOVs_PlotFig(lobVon,lobVmov,lobVrewT,'Lobule V',{'Trial onset','Wheel movement','Trial Reward'},{'Learning','Expert'});
figV_RewRCT = fig3_matchFOVs_PlotFig(lobVrewR,lobVrewC,lobVrewT,'Lobule V',{'Random reward','Cued reward','Trial Reward'},{'Learning','Expert'});
...
%% Save maybe
if savebool
    figFold = fullfile(paperFold,'Figures','Fig3','Sub2_v44');
    if ~exist(figFold,'dir'); mkdir(figFold); end
    save(fullfile(figFold,'match_v4_v4.mat'),'matchV14');
