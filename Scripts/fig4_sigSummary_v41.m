function fig4_sigSummary_v41(sessions,dataFold,paperFold,glmParams,savebool)
%%
glmFold = sprintf('GLMfull_%s_%s_alpha%i',glmParams.dataType,glmParams.distr,glmParams.alpha*100);
figFold0 = fullfile(paperFold,'Figures','Fig4','Sub3_SummaryBars_v4learning');
...
for iSesh = 1:numel(sessions)
    ...
    glmSummary(iSesh).pPCs_vis = glmSummary(iSesh).nSigVis/glmSummary(iSesh).nPC*100;
    glmSummary(iSesh).nSigMov = sum(cat(1,glmLocal.sigBool_shufMov_proj));
    glmSummary(iSesh).pPCs_mov = glmSummary(iSesh).nSigMov/glmSummary(iSesh).nPC*100;
    glmSummary(iSesh).nSigRew = sum(cat(1,glmLocal.sigBool_shufRew_proj));
    glmSummary(iSesh).pPCs_rew = glmSummary(iSesh).nSigRew/glmSummary(iSesh).nPC*100;
    glmSummary(iSesh).nSigLick = sum(cat(1,glmLocal.sigBool_shufLick_proj));
    glmSummary(iSesh).pPCs_lick = glmSummary(iSesh).nSigLick/glmSummary(iSesh).nPC*100;
...
if savebool
    save(fullfile(figFold,'sigSummary_v41.mat'),'glmSummary');
end
...
if savebool
    ...
    print(fJoint,fullfile(figFold,[savename,'.eps']), '-depsc', '-painters');
end
...
if savebool
    ...
    print(fDevExp,fullfile(figFold,[savename,'.eps']), '-depsc', '-painters');
end
...
if savebool
    ...
    print(flobV,fullfile(figFold,[savename,'.eps']), '-depsc', '-painters');
end
...
if savebool
    ...
    print(fsim2,fullfile(figFold,[savename,'.eps']), '-depsc', '-painters');
end
...
e1.CapSize = 0; e1.LineWidth = 3.5; e1.Marker = 'none'; e1.Color = 'k';
text(1.5,100,sprintf('p = %.2f',bVis),'FontSize',12,'HorizontalAlignment','center')
...
e1.CapSize = 0; e1.LineWidth = 3.5; e1.Marker = 'none'; e1.Color = 'k';
text(4.5,100,sprintf('p = %.2f',bMov),'FontSize',12,'HorizontalAlignment','center')
...
e1.CapSize = 0; e1.LineWidth = 3.5; e1.Marker = 'none'; e1.Color = 'k';
text(7.5,100,sprintf('p = %.2f',bRew),'FontSize',12,'HorizontalAlignment','center')
...
e1.CapSize = 0; e1.LineWidth = 3.5; e1.Marker = 'none'; e1.Color = 'k';
text(10.5,100,sprintf('p = %.2f',bLic),'FontSize',12,'HorizontalAlignment','center')
...
if savebool
    ...
    print(fboth,fullfile(figFold,[savename,'.eps']), '-depsc', '-painters');
end
