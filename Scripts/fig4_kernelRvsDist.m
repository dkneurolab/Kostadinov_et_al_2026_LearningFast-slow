function fig4_kernelRvsDist(seshv4, paperFold, savebool)
...
for isesh = 1:numel(GLMv144)
    ...
    iPC_v4_v1_all = cat(2,GLMv144(isesh).GLMmatchv4n_v1.iPC_v4);
    ...
    plotType = 'mov';
    kernelMatv1 = cat(2,GLMv144(isesh).(sprintf('%sKernel_v1',plotType)));
    ...
    if sum(iGood) > 0
        ...
        Rmat = corrcoef([kernelMatv1 kernelMatv4n]);
        Rmat_compare = Rmat(1:nPCs,nPCs+1:end);
        Rmat_matched0 = Rmat_compare(1:nPCs+1:end)';

        % Compute cell IDs and locations for same cells
        sigFits = cat(2,GLMv144(isesh).GLMmatchv4n_v1.sigBool_shufMov);
        xCtrInds0 = iPC_v4_v1_all(~isnan(sigFits));
    ...
    if sum(iGood) > 0
        ...
        % Compute cell IDs and locations for same cells
        sigFits = cat(2,GLMv144(isesh).GLMmatchv4n_v1.sigBool_shufRew);
        xCtrInds0 = iPC_v4_v1_all(~isnan(sigFits));
        xCtrInds0 = xCtrInds0(iGood);
        xCtr0 = xCtrPx(xCtrInds0)*670/512;
...
suptitle('Distance-dependence of kernel correlations: Naive-expert');
...
%% Save maybe
if savebool
    figname = 'kernelRvsDist_v1_v4n_all';
    ...
    figname = 'kernelRvsDist_v1_v4n_lobv';
    ...
    figname = 'kernelRvsDist_v1_v4n_sim2';
    ...
    figname = 'kernelRvsDist_v1_v4n_summary';
    ...
    GLMdistance_v1_v4 = rmfield(GLMv144,{'GLMmatchv4n_v1', 'GLMmatchv4n_v41'});

    save(fullfile(glmFold,'GLMdistance_v1_v4.mat'),'GLMdistance_v1_v4');
