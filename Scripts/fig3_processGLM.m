function fig3_processGLM(GLMroot, glmParams, savebool)
...
%% Load in data
GLMdata = load(fullfile(GLMroot,'GLMfull_small.mat'));
v2struct(GLMdata.GLMfull_small);
...
iFoldsmall = readmatrix(fullfile(GLMroot,'FullData_small','iFoldSmall.csv'));
...
%% Run full GLM
glmFile = fullfile(glmFold,'GLMsig.mat');
...
% Visual predictors
glmFile = fullfile(glmFold,'GLMsig+Vis.mat');
if ~exist(glmFile,'file')
    fprintf('Mouse name:       %s\n',fPathSplit{5});
    fprintf('Session date+FOV: %s\n',fPathSplit{6});
...
% Movement predictors
glmFile = fullfile(glmFold,'GLMsig+VisMov.mat');
if ~exist(glmFile,'file')
    fprintf('Mouse name:       %s\n',fPathSplit{5});
    fprintf('Session date+FOV: %s\n',fPathSplit{6});
...
% Reward predictors
glmFile = fullfile(glmFold,'GLMsig+VisMovRew.mat');
if ~exist(glmFile,'file')
    fprintf('Mouse name:       %s\n',fPathSplit{5});
    fprintf('Session date+FOV: %s\n',fPathSplit{6});
...
% Lick predictors
glmFile = fullfile(glmFold,'GLMsig+VisMovRewLick.mat');
if ~exist(glmFile,'file')
    fprintf('Mouse name:       %s\n',fPathSplit{5});
    fprintf('Session date+FOV: %s\n',fPathSplit{6});
