function fig3_matchGLMbars(sessionsv1,v1,sessionsv4_n,v2,trialType,glmParams,dataFold,paperFold,savebool)
...
for iSesh = 1:numel(sessionsv4_n)    
    for jSesh = 1:numel(sessionsv1)
        if strcmpi(sessionsv4_n(iSesh).name,sessionsv1(jSesh).name) && ...
            ...
            switch trialType
                case 'corrects'
                    GLMoutTemp = fig3_matchGLMcorrectTrials(sessionsv4_n(iSesh),sessionsv1(jSesh),glmParams,dataFold);
                case 'all'
                    GLMoutTemp = fig3_matchGLMallTrials(sessionsv4_n(iSesh),sessionsv1(jSesh),glmParams,dataFold);
            end
            
            GLMout(aa).name = sessionsv1(jSesh).name;
            if strcmpi(sessionsv1(jSesh).fov,'lobv')
                GLMout(aa).ilobv = true;
                GLMout(aa).isim2 = false;
            end
            if strcmpi(sessionsv1(jSesh).fov,'sim2')
